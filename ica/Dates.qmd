...
Birthdays %>%
  group_by(state) %>%
  summarise(total_births_per_state = sum(births)) %>%
  arrange(total_births_per_state)
...
```{r}
ggplot(daily_births, aes(x = day, y = total_births, color = day_of_week)) +
  geom_point()
```
...
```{r}
births_monthly <- Birthdays |> 
  mutate(month = month(date, label = TRUE, abbr = TRUE),
         year = year(date)) |> 
  group_by(year, month) |> 
  summarise(total_births = sum(births), .groups = "drop")

ggplot(births_monthly, aes(x = month, y = total_births, group = year)) +
  geom_line(alpha = 0.5) 
```
...
```{r}
births_weekly <- Birthdays |> 
  filter(year == 1988, week != 53) |> 
  group_by(state, week) |> 
  summarise(total_births = sum(births), .groups = "drop")

ggplot(births_weekly, aes(x = week, y = total_births, color = state)) +
...
```{r}
births_mn_la <- Birthdays |> 
  filter(year == 1988, week != 53, state %in% c("MN", "LA")) |> 
  group_by(state, week) |> 
  summarise(total_births = sum(births), .groups = "drop")

ggplot(births_mn_la, aes(x = week, y = total_births, color = state)) +
  geom_line()
```
