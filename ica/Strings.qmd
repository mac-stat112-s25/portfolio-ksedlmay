...
classes <- classes %>%
  mutate(is_spring = str_detect(sem, "SP"))
...
classes <- classes %>%
  mutate(sem = str_replace_all(sem, c("SP" = "spring", "FA" = "fall")))
```
...
courses_clean <- courses |> 
  separate(avail_max, into = c("avail", "max"), sep = "/", convert = TRUE) |> 
  mutate(enroll = as.numeric(max) - as.numeric(avail)) |> 
  separate(number, into = c("dept", "number", "section"), sep = " ", extra = "merge", fill = "right")
...
top_depts_sections <- courses_clean |> 
  count(dept, name = "num_sections") |> 
  arrange(desc(num_sections)) |> 
  head(6)

print(top_depts_sections)

# Identify the 6 departments with the longest average course titles
top_depts_title_length <- courses_clean |> 
  mutate(title_length = str_length(name)) |> 
  group_by(dept) |> 
  summarize(avg_title_length = mean(title_length, na.rm = TRUE)) |> 
  arrange(desc(avg_title_length)) |> 
  head(6)

print(top_depts_title_length)
```
...
alicia_courses <- courses_clean |> 
  filter(str_detect(name, "Alicia Johnson"))
...
stat <- courses_clean |> 
  ...
  mutate(
    name = str_replace(name, "Introduction to ", ""),  # Remove "Introduction to"
    name = str_replace(name, "Statistical", "Stat"),    # Shorten "Statistical" to "Stat"
    start_time = str_sub(time, 1, 5)                    # Extract the start time (first 5 characters of 'time')
  ) |> 
  select(number, name, start_time, enroll)  # Keep only the required columns
