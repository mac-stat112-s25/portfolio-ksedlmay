...
Summarize your observations from the plot. Comment on the basics: range, typical outcomes, shape. (Any theories about what might explain this non-normal shape?)
  ...
  The histogram shows a right skew indicating that here are more schools that have a lower sat score than a higher.
...
ggplot(education, aes(x = salary, y = expend)) +
  geom_point() +
...
What are the relationship trends between SAT scores and spending? Is there anything that surprises you?
  ...
  There are no trends between sat and spending based on teacher salary. Expend and salary have a proportional relationship and a positive slope whilst the salary and sat scores have a slightly negative relationship.
...
```{r}
ggplot(education, aes(x = sat, y = salary, col = expend)) +
  geom_point() +
...
Construct the plot below. Check out the code and think about what's happening here. What happens if you change "2" to "3"?
  ...
  The only this that changes is you get 3 ranges of colors rather than 2, this cut function cuts the data into ranges that are easier to read.
...
Describe the trivariate relationship between `sat`, `salary`, and `expend`.
  ...
  The relationship is negative, there is a negative correlation between salary and sat scores. Also looks like the more a school spends, the worse the sat score is.
...
```{r}
ggplot(education) +
  geom_bar(aes(x = fracCat, color = cut(expend, 3))) +
  geom_jitter( aes(x = fracCat, y = sat, color = cut(expend,3)))
...
Putting all of this together, explain this example of **Simpson’s Paradox**. That is, why did it appear that SAT scores decrease as spending increases even though the *opposite* is true?
  ...
  Wealthier states have higher sat scores but less participation, but higher spending lead to more participation and lower scores. 
