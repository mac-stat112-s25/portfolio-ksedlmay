...
ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9am", y = "Temperature at 3pm", title = "Comparing the Temperature of Australian Cities", color = "Cities")  
```
...
```{r}
#| fig-cap: "Temperature Relationship at 9 AM vs. 3 PM by Location"

ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9am", y = "Temperature at 3pm", color = "Cities")  
```
...
```{r}
#| fig-cap: "Density plots of 3pm temperatures in 3 Australian locations."
#| fig-alt: "A density plot showing the distribution of 3pm temperatures in Hobart, Uluru, and Wollongong, Australia. Uluru has the highest temperatures on average, while Hobart has the lowest."
...
-   Summarize what you learn. What impact might our color choices have on one's ability to interpret the viz?
  ...
  When I ran the plot through a color blindness simulator, I noticed that some colors were hard to distinguish, which could make it difficult to compare the distributions. To improve accessibility, I’d use a colorblind-friendly palette like viridis or RColorBrewer, or add patterns and line variations for better differentiation.
